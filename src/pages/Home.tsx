    ...
    <div>
      <div className="flex items-center justify-between px-5 py-3 border-b border-[#1a1a1a]">
        <div className="flex gap-2">
          {(['day', 'week'] as TrendWindow[]).map(w => (
            <button
              key={w}
              onClick={() => setWindow(w)}
              className={`boton-elegante boton-sm ${window === w ? 'boton-active' : ''}`}
            >
              {w === 'day' ? 'Hoje' : 'Semana'}
            </button>
          ))}
        ...
        <div className="flex gap-2">
          {([['all', 'Tudo'], ['movie', 'Filmes'], ['tv', 'Séries']] as [TrendFilter, string][]).map(([val, label]) => (
            <button
              key={val}
              onClick={() => setFilter(val)}
              className={`boton-elegante boton-sm ${filter === val ? 'boton-active' : ''}`}
            >
              {label}
            </button>
          ))}
